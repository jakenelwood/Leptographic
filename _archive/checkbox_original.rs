        ...
        <Provider value=context_value>
            <div class="relative inline-flex">
                ...
                <button
                    ...
                    class=move || {
                        let base_classes = "relative inline-flex items-center justify-center w-6 h-6 rounded bg-white appearance-none cursor-pointer transition-all duration-150 ease-out focus:outline-none";

                        let state_classes = match (current_checked.get(), disabled.get()) {
                            // Disabled state - gray background, no border, no hover
                            (_, true) => "bg-gray-200 cursor-not-allowed opacity-60",
                            // Checked state - white bg with black ring (outer border), no hover
                            (CheckedState::True | CheckedState::Indeterminate, false) => "bg-white ring-2 ring-black",
                            // Unchecked state - white bg, no border, custom purple hover only when enabled
                            (CheckedState::False, false) => "bg-white hover:bg-[#F4F0FE]",
                        };
...
pub fn CheckboxIndicator(
    ...
    view! {
        <Show when=move || is_present.get()>
            <div
                class=move || {
                    let class_value = class.get();
                    format!("leptonic-checkbox-indicator absolute inset-0 flex items-center justify-center text-black font-bold text-sm leading-none pointer-events-none {class_value}")
                }
                data-state=get_state
